        ...
        <v-container class="pa-4 text-center">
          <v-row class="fill-height flex-row box" align="center" justify="space-around">
            <template>
                    ...
                    <v-toolbar-title>Blocked List</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-btn
                    icon
...
export default {
  data: () => ({
    ...
    computed: {
      ...
      allSelected1 () {
        return this.selected.length === this.blocks.length
      },
      categories1 () {
        const search = this.search.toLowerCase()

        if (!search) return this.blocks
      ...
      selections1 () {
        ...
        for (const selection1 of this.selected) {
