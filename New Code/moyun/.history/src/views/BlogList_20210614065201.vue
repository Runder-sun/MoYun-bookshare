    ...
    <v-app>
        ...
        <el-table
            ...
            <template slot-scope="scope">
                <v-btn text color="cyan lighten-1" rounded @click="handleEdit(scope.$index, scope.row)"> 查看 </v-btn>
...
export default({
    ...
    created(){
      ...
      handleEdit(index, row) {
        this.$router.push({path:'/BlogList/ScanBlog',query: {id:"tableData[index].blogID"}});
      },
      handleDelete(index, row) {
        this.$http({
          ...
          if(res.data.success===1){
            this.$router.push({path:"/BlogList",query:{id:" this.tableData[index].blogID "}});
          }
          else{
            alert("删除失败！");
          }
